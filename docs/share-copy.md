...
It is meant for workflows that still rely on implicit defaults, `read-all`, `write-all`, or workflow-level write permissions. It runs offline, does not call the GitHub API, and prints copy-paste snippets.
...
I built Permissio: least privilege for GitHub Actions tokens in one command.

It scans `.github/workflows` and recommends explicit job-level `GITHUB_TOKEN` permissions.
...
- Table, Markdown, JSON, and HTML reports.
